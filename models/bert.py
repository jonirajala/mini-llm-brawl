...
class Bert(nn.Module):
    ...
    def forward(self, x, seg, attn_mask=None):
        ...
        # Masking
        if self.training:
            x, masked_labels = create_masked_lm_predictions(
                x,
    ...
    def generate(self, inp, temperature=1.0, top_k=None):
        inp = tokenizer(inp, return_tensors="pt").to(device)
...
def create_masked_lm_predictions(tokens, vocab_size, mask_token_id, pad_token_id):
    ...
    indices_replaced = (
        torch.bernoulli(torch.full(labels.shape, 0.8).to(device)).bool()
        & masked_indices
    )
    ...
    indices_random = (
        torch.bernoulli(torch.full(labels.shape, 0.5).to(device)).bool()
        & masked_indices
        & ~indices_replaced
    )
